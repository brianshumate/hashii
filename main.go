...
func init() {
	...
	flag.BoolVar(&plainMode, "plain", false, "Display logo without color")
	flag.Usage = customUsage // Add this line to set a custom usage function
...
func customUsage() {
	fmt.Printf("Usage of hashii v%s:\n", version)
	fmt.Println("  hashii [options]")
	fmt.Println("  hashii version")
	fmt.Println("  hashii help")
	fmt.Println("\nOptions:")
	flag.PrintDefaults()
}
